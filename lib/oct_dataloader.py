...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def fetch(self, possibly_batched_index):
        ...
        for idx in possibly_batched_index:
            image_paths = self._dataset[0][idx]
            thedata = image_read3d(image_paths)
            thedata = self.img1_trans(thedata)
            data[0].append(thedata)
            data[1].append(self._dataset[1][idx])
            data[2].append(self._dataset[2][idx])
        ...
        # 对标签处理
        data[1] = torch.tensor(np.array(data[1]))
