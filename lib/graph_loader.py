...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def fetch(self, possibly_batched_index):
        ...
        # 对标签处理
        data[0] = torch.tensor(np.array(data[0]))
        data[0] = data[0].type(torch.LongTensor)
...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset, batch_size: Optional[int] = 1,
        ...
        if spmod == 'seq':
            sampler = SequentialSampler(dataset[0])
        else:
            sampler = RandomSampler(len(dataset[0]))  # 初始化RandomSampler
