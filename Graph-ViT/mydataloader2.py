...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def fetch(self, possibly_batched_index):
        ...
        for idx in possibly_batched_index:
            data[0].append(self._dataset[0][idx])
            data[1].append(torch.from_numpy(self._dataset[1][idx]))
            data[2].append(torch.from_numpy(self._dataset[2][idx]))
            data[3].append(torch.from_numpy(self._dataset[3][idx]))
            data[4].append(self._dataset[4][idx])

        data[1] = torch.stack(data[1], 0)
        data[1] = data[1].type(torch.FloatTensor)

        data[2] = torch.stack(data[2], 0)
        data[2] = data[2].type(torch.FloatTensor)

        data[3] = torch.stack(data[3], 0)
        data[3] = data[3].type(torch.FloatTensor)

        # 对标签处理
        data[4] = torch.tensor(np.array(data[4]))
        data[4] = data[4].type(torch.LongTensor)
...
class DataLoader(Generic[T_co]):
    ...
    def __len__(self) -> int:
        ...
        return ceil(len(self.dataset[1])/ self.batch_size)