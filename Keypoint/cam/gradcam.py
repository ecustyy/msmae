...
class GradCAM(BaseCAM):
    ...
    def forward(self, x, class_idx=None, retain_graph=False):
        ...
        self.model.zero_grad()  #清除self.gradients
        score.backward(retain_graph=retain_graph)

        gradients = self.gradients['value'].data    #[1,512,14,14]
        activations = self.activations['value'].data    #[1,512,14,14]
        b, k, u, v = activations.size()

        gradients = F.relu(gradients)
        activations = F.relu(activations)

        alpha = gradients.view(b, k, -1).mean(2)    #[1,512]    把最后两个维度平均
        weights = alpha.view(b, k, 1, 1)
        saliency_map = (weights * activations).sum(1, keepdim=True) #对通道求和

        # saliency_map = F.relu(saliency_map)
        saliency_map = F.interpolate(saliency_map, size=(h, w), mode='bilinear', align_corners=False)
        saliency_map_min, saliency_map_max = saliency_map.min(), saliency_map.max() #最大最小归一化
